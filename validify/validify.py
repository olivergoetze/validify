    ...
    if element_name in validation_rules:
        ...
        for validation_rules_set in validation_rules[element_name]:
            ...
            if not validation_rules_set["element_children_optional"]:
                if len(element) == 0:
                    message_text = "Element {} enthält keine Subelemente, obwohl eines oder mehrere Subelemente erwartet.".format(element_name)
                    validation_messages.append(message_text)
                    validation_results.append({"message_id": "0001", "message_text": message_text, "element_name": element_name, "element_sourceline": element_sourceline})

            # element content optional
            if not validation_rules_set["element_content_optional"]:
                if get_compare_value(element) == "":
                    message_text = "Element {} enthält keinen Elementinhalt, obwohl das Element nicht leer sein darf.".format(element_name)
                    validation_messages.append(message_text)
                    validation_results.append(
                        {"message_id": "0002", "message_text": message_text, "element_name": element_name, "element_sourceline": element_sourceline})
            ...
            for element_attribute in element_attributes:
                if (element_attribute not in validation_rules_set["optional_attributes"]) and (element_attribute not in validation_rules_set["obligatory_attributes"]):
                    message_text = "Element {} enthält ein nicht erwartetes Attribut: {}".format(element_name, element_attribute)
                    validation_messages.append(message_text)
                    validation_results.append(
                        {"message_id": "0003", "message_text": message_text, "element_name": element_name, "element_sourceline": element_sourceline})
            ...
            for obligatory_attribute in validation_rules_set["obligatory_attributes"]:
                if obligatory_attribute not in element_attributes:
                    message_text = "Element {} enthält nicht das Pflicht-Attribut {}.".format(element_name, obligatory_attribute)
                    validation_messages.append(message_text)
                    validation_results.append(
                        {"message_id": "0004", "message_text": message_text, "element_name": element_name, "element_sourceline": element_sourceline})
            ...
            for element_subelement in element_subelements:
                if (element_subelement not in validation_rules_set["optional_subelements"]) and (element_subelement not in validation_rules_set["obligatory_subelements"]):
                    message_text = "Element {} enthält ein nicht erwartetes Subelement: {}.".format(element_name, element_subelement)
                    validation_messages.append(message_text)
                    validation_results.append(
                        {"message_id": "0005", "message_text": message_text, "element_name": element_name, "element_sourceline": element_sourceline})
            ...
            for obligatory_subelement in validation_rules_set["obligatory_subelements"]:
                if obligatory_subelement not in element_subelements:
                    message_text = "Element {} enthält nicht das Pflicht-Subelement {}.".format(element_name, obligatory_subelement)
                    validation_messages.append(message_text)
                    validation_results.append(
                        {"message_id": "0006", "message_text": message_text, "element_name": element_name, "element_sourceline": element_sourceline})
            ...
            if validation_rules_set["max_occurence"] is not None:  # max occurence nur prüfen, wenn das Element nicht unbegrenzt auftreten kann
                ...
                if len(element_siblings) > validation_rules_set["max_occurence"]:
                    message_text = "Element {} ist {} mal vorhanden, erwartet wird jedoch nur {} Vorkommen.".format(element_name, len(element_siblings), validation_rules_set["max_occurence"])
                    validation_messages.append(message_text)
                    validation_results.append(
                        {"message_id": "0007", "message_text": message_text, "element_name": element_name, "element_sourceline": element_sourceline})

            # character content allowed
            if validation_rules_set["text_character_content_allowed"] is False:
                if element.text is not None:
                    if element.text != "":
                        message_text = "Element {} should not contain text character content.".format(element_name)
                        validation_messages.append(message_text)
                        validation_results.append(
                            {"message_id": "0008", "message_text": message_text, "element_name": element_name, "element_sourceline": element_sourceline})
            if validation_rules_set["tail_character_content_allowed"] is False:
                if element.tail is not None:
                    if element.tail != "":
                        message_text = "Element {} should not contain tail character content.".format(element_name)
                        validation_messages.append(message_text)
                        validation_results.append(
                            {"message_id": "0009", "message_text": message_text, "element_name": element_name, "element_sourceline": element_sourceline})
            ...
            if len(validation_rules_set["allowed_values"]) > 0:
                if element.text is not None:
                    if element.text not in validation_rules_set["allowed_values"]:
                        message_text = "{} -- Element value not eqal to required value: '{}' != '{}'".format(element_name, element.text, ", ".join(validation_rules_set["allowed_values"]))
                        validation_messages.append(message_text)
                        validation_results.append({"message_id": "0010", "message_text": message_text, "element_name": element_name, "element_sourceline": element_sourceline})
            ...
            for attribute_definition in validation_rules_set["attribute_def"]:
                if attribute_definition["attribute_name"] in element.attrib:
                    if element.attrib[attribute_definition["attribute_name"]] not in attribute_definition["allowed_values"]:
                        message_text = "{}, attribute {} -- attribute value not eqal to required value: '{}' != '{}'".format(element_name, attribute_definition["attribute_name"], element.attrib[attribute_definition["attribute_name"]], ", ".join(attribute_definition["allowed_values"]))
                        validation_messages.append(message_text)
                        validation_results.append({"message_id": "0011", "message_text": message_text, "element_name": element_name, "element_sourceline": element_sourceline})
